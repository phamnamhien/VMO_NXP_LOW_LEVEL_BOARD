...
#define GMAC_IP_SW_MAJOR_VERSION_C               6
    ...
    if (((Bd->Des3 & GMAC_RDES3_FD_MASK) != 0U))
    {
        ...
        if (GMAC_STATUS_SUCCESS == Status)
        {
            ...
            if (TRUE == EndOfQueueReached)
            {
                ...
                LengthToCopy = Buff->Length - (CountOfBdUntilQueueEnd * DescriptorDataBuffLen);
                DataBufferSrc = (const uint8 *)(Gmac_Ip_PtrSizeType)ListBd->Des0;
                DataBufferDest = (uint8 *)(Gmac_Ip_PtrSizeType)LastBd->Des0;
