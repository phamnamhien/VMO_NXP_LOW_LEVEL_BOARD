...
#define STM_IP_SW_MAJOR_VERSION_C             6
...
uint32 Stm_Ip_u32TargetValue[GPT_STM_INSTANCE_COUNT][STM_CHANNEL_COUNT];
        ...
        defined(CPE_STM_0_ISR_USED) || defined(CPE_STM_1_ISR_USED) || defined(LPE_STM_ISR_USED) || defined(DSP4_STM_ISR_USED) || \
...
#if defined(CPE_STM_0_ISR_USED)
ISR(CPE_STM_0_ISR);
#endif
#if defined(CPE_STM_1_ISR_USED)
ISR(CPE_STM_1_ISR);
#endif
...
/**
* @brief   Interrupt handler for STM channels.
* @details Interrupt Service Routine corresponding to common CPE_STM_0 module.
* @param[in] none
* @return  none
* @isr
* @pre      The driver needs to be initialized.
...
ISR(CPE_STM_0_ISR){
    ...
    #error "undefined STM instance number"
#endif
    for (channel = 0U; channel < CPE_STM_0_IP_CHANNELS_NUMBER; ++channel)
    {
        /* check if channel event has occurred */
        HasChEvOccurred = Stm_Ip_GetInterruptStatusFlag(instance, channel);
        if (TRUE == HasChEvOccurred)
...
/**
* @brief   Interrupt handler for STM channels.
* @details Interrupt Service Routine corresponding to common CPE_STM_1 module.
* @param[in] none
* @return  none
* @isr
* @pre      The driver needs to be initialized.
...
ISR(CPE_STM_1_ISR){
    ...
    #error "undefined STM instance number"
#endif
    for (channel = 0U; channel < CPE_STM_1_IP_CHANNELS_NUMBER; ++channel)
    {
        /* check if channel event has occurred */
        HasChEvOccurred = Stm_Ip_GetInterruptStatusFlag(instance, channel);
        if (TRUE == HasChEvOccurred)
